...
class Data:
    ...
    def _init_it(self):
        ...
        self._food_name_to_id = {row['food_name']: row['food_id'] for row in self._read_tsv(
            f'{DATA_DIR}/name_foodID_correspondence.tsv')}

    def _init_en(self):
        self._food_data_path = 'food_details_EN'
        self._food_name_to_id = {row['food_name']: row['food_id'] for row in self._read_tsv(
            f'{DATA_DIR}/name_foodID_correspondence_EN.tsv')}
