...
def open_github():
    webbrowser.open_new(
        "https://github.com/FabbriniMarco/MacroMicro-ITA-Tracker")
...
class MacroMicro(ttk.Frame):
    ...
    micronutrient_totals = {
        micronutrient: 0.0 for micronutrient in micronutrient_list}
    added_foods = []
    ...
    def export_data(self):
        ...
        data_to_export = [{'micronutrient': k, 'total_amount': v}
                          for k, v in self.micronutrient_totals.items()]
    ...
    def add_food(self):
        ...
        try:
            ...
            messagebox.showerror(
                "Error", f"Details file for food ID {food_id} not found.")
            return
        except Exception as e:
            messagebox.showerror(
                "Error", f"An error occurred while reading the food details: {e}")
            return

        actual_amounts = {}
        for micronutrient in self.micronutrient_totals:
            if micronutrient in food_details:
                actual_amounts[micronutrient] = food_details[micronutrient] * \
                    (quantity / 100.0)
        ...
        for micronutrient, amount in actual_amounts.items():
            ...
            if micronutrient_index in self.proplipids:
                self.micronutrient_totals[micronutrient] += (
                    food_details[micronutrient] / 100.0) * lipid_total
            elif micronutrient_index in self.propaa:
                self.micronutrient_totals[micronutrient] += (
                    food_details[micronutrient] / 100.0) * aa_total
            else:
    ...
    def update_added_foods_list(self):
        ...
        for i, (food_name, quantity) in enumerate(self.added_foods):
            ...
            frame.pack(anchor='w')
            ttk.Label(
                frame, text=f"{food_name} - {quantity}g").pack(side='left')
            ttk.Button(frame, text="Duplicate", command=lambda i=i: self.duplicate_food(
                i), bootstyle='secondary-outline').pack(side='right', padx=5)
            ttk.Button(frame, text="Delete", command=lambda i=i: self.delete_food(
                i), bootstyle='danger-outline').pack(side='right', padx=5)
    ...
    def wipe_data(self):
        if messagebox.askyesno("Confirm Wipe", "Are you sure you want to wipe all data? Unsaved data will be lost."):
            self.micronutrient_totals = {
                micronutrient: 0.0 for micronutrient in self.micronutrient_list}
            self.added_foods = []
            self.update_added_foods_list()
            self.food_var.set('')
    ...
    def show_info(self):
        ...
        text_widget = ttk.Text(info_window, wrap="word",
    ...
    def __init__(self, master: ttk.Window):
        ...
        language_menu = ttk.Combobox(self, textvariable=self.current_language, values=[
                                     "Italian", "English"], state="readonly", bootstyle="dark", width=6)
        ...
        self.quantity_entry = ttk.Entry(
            self, textvariable=self.quantity_var, width=50)
        ...
        scrollbar = ttk.Scrollbar(
            self.added_foods_container, orient="vertical", command=canvas.yview)
        ...
        canvas.create_window(
            (450, 50), window=scrollable_frame, anchor="center")
        ...
        wipe_button = ttk.Button(
            self, text="Wipe Data", command=self.wipe_data, bootstyle='danger')
        ...
        info_button = ttk.Button(
            button_frame, text="Info", command=self.show_info, bootstyle='secondary')
        info_button.pack(side="left", padx=5)

        website1_button = ttk.Button(
            button_frame, text="Github", command=open_github, bootstyle='secondary')
